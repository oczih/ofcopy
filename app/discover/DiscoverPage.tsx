...
export default function App({ creators, session }: AppProps) {
  ...
  // Filter only discoverable creators
  const discoverableCreators = creators.filter((c) => c.isDiscoverable);

  // Search filter
  const filteredCreators = discoverableCreators.filter((creator) =>
    creator.name.toLowerCase().includes(searchQuery.toLowerCase()) ||
    creator.username.toLowerCase().includes(searchQuery.toLowerCase()) ||
    creator.category.toLowerCase().includes(searchQuery.toLowerCase())
  );

  // Handle "no results" delay
  useEffect(() => {
    if (!loading && filteredCreators.length === 0) {
      const timer = setTimeout(() => setShowNoResults(true), 1500);
      return () => clearTimeout(timer);
    } else {
        ...
        <div className="space-y-8 animate-fade-in">
            ...
            <div className="flex items-center gap-4 mb-6">
              ...
              <div>
                ...
                <p className="text-gray-400 mt-1">
                  Find amazing content creators across all categories
                </p>
              </div>
            </div>
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
              {loading
                ? Array.from({ length: 4 }).map((_, i) => <TrendingCreatorSkeleton key={i} />)
                : filteredCreators.slice(0, 4).map((creator, i) => (
                    <div
                      key={creator._id}
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-white text-sm font-medium">4.{9 - i}</span>
                        </div>
                      </div>
                    </div>
                  ))}
          ...
          <div className="bg-white/5 backdrop-blur-xl rounded-3xl p-8 border border-white/10 shadow-2xl">
            <div className="flex items-center justify-between mb-6">
              <h2 className="text-2xl font-bold text-white">Our current creators</h2>
