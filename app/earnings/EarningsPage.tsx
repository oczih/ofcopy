...
export default function App({ creators, session, users }: AppProps) {
  ...
  useEffect(() => {
    if (session) {
      ...
      setCreator(creatorCorrect || null);
    } else {
      setCreator(null);
    }
  }, [session, creators]);

  // Map subscribers to Payment objects
  const payments: Payment[] = useMemo(() => {
    if (!creator?.subscribers) return [];
    return creator.subscribers.map(s => ({
      id: s.userId.toString(),
      amount: s.subscriptionPrice,
      date: new Date(s.subscribedAt), // ensure it's a Date
      type: 'subscription',
      user: users.find(u => u._id === s.userId.toString())
    }));
  }, [creator]);
  ...
  const groupedPayments = useMemo(() => {
    ...
    return sorted.reduce((acc: Record<string, Payment[]>, payment) => {
  ...
  .map(([date, dayPayments]) => (
    <div key={date} className="space-y-3">
      <h4 className="text-gray-300 font-medium">{formatDate(new Date(date))}</h4>
          ...
          >
            <div>
              <div className="flex items-center gap-2">
                <span className="text-white capitalize font-semibold">{p.type} from {p.user?.username || "Deleted User"}</span>
                <span className="text-sm text-gray-400">{formatTime(p.date)}</span>
              </div>
              <p className="text-sm text-gray-300">Gross: ${p.amount?.toFixed(2) || 0}</p>
              <p className="text-sm text-gray-300">Fanslio Fee (10%): ${(p.amount * 0.1).toFixed(2)}</p>
              <p className="text-xs text-gray-400 mt-1">
                Payout available on {formatDate(payoutDate)}
              </p>
            </div>

            <p className="text-lg text-green-400 font-semibold">${net.toFixed(2)}</p>
          </div>
        );
      })}
    </div>
